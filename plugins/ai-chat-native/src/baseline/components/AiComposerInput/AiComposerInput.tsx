/**
 * AiComposerInput — the chat composer textarea with inline snippet/command
 * (`#` / `/`) and file (`@`) pickers. Owns the trigger state machine, picker
 * keyboard navigation, and insertion logic. Pure trigger scanners live in
 * `./triggers`.
 */

import { Popover, PopoverAnchor, usePresence } from "@termco/ui";
  ...
  return (
    <>
      <Popover
        open={pickerOpen}
        onOpenChange={(open) => {
          // Popover open is derived from the caret trigger; when Radix asks to
          // close (Escape / click-outside) clear the trigger so it can't linger
          // open with its portal floating over the header.
          if (!open) {
            setTrigger(null);
            setFileTrigger(null);
          }
      ...
      >
        <PopoverAnchor asChild>
          <div className="w-full">
            <textarea
              data-onboarding-target="ai-chat.composer"
              ref={c.textareaRef}
              value={c.value}
              ...
              rows={2}
              className={cn(
                "block min-h-12 max-h-40 w-full resize-none bg-transparent text-sm leading-relaxed outline-none",
      ...
      {voiceRow.mounted && (
        <div data-state={voiceRow.state} className="termco-reveal">
...
function autoresize(el: HTMLTextAreaElement | null) {
  ...
  // Keep an empty field at one row — a long placeholder can wrap and inflate
  // scrollHeight, which used to make the box render several lines tall.
  el.style.height = el.value ? `${Math.min(el.scrollHeight, 160)}px` : "";
}
