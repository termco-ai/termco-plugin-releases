// useMiniWindowGeometry: drives the mini window's position/size purely through
// the DOM (no React state) so streaming re-renders can't disturb an in-flight
// drag/resize gesture; writes are batched into one rAF per frame.
import { pingOverlays } from "@termco/ui";
...
import { type Compute, loadGeom, saveGeom, viewport } from "./persistence";
...
export function useMiniWindowGeometry() {
  ...
  const flush = useCallback(() => {
    ...
    // The window moved without opening/closing — let a native browser view
    // recompute whether it's now covered by (or clear of) this window.
    pingOverlays();
  }, []);
