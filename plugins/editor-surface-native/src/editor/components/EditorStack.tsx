/**
 * `EditorStack` — layers all open, warm editor tabs and shows the active one.
 *
 * Keeps every non-cold editor tab mounted (hidden when inactive) so switching
 * tabs is instant and scroll/undo state survives, and hands each `EditorPane` a
 * per-tab-stable set of ref/dirty/close callbacks. Markdown files also get a
 * view-mode toggle.
 */
import { cn, isMarkdownPath } from "../../utils";
import type { EditorTab, Tab } from "../../tabs";
...
type Props = {
  ...
  /** Open (or focus) a file at a position — go-to-definition target. */
