...
import { currentWorkspaceEnv, type WorkspaceEnv } from "../../workspace";
...
type Options = {
  path: string;
  /** The env of the owning tab's rig; falls back to the global env when the
   * caller predates per-tab threading. */
  env?: WorkspaceEnv;
...
export function useDocument({ path, env, onDirtyChange }: Options) {
  ...
  const workspace = useCallback(
    () => envRef.current ?? currentWorkspaceEnv(),
    [],
  );
  ...
  const save = useCallback(async () => {
    clearAutoSaveTimer();
    // Compare via refs, not the `dirty` state: a synchronous buffer change
    // immediately before save (format-on-save) hasn't re-rendered yet, so the
    // state in this closure would still say "clean" and skip the write.
