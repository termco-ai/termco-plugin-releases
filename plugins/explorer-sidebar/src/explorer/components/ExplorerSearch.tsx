/**
 * Debounced fuzzy file-search overlay for the explorer sidebar.
 * Owns the query input, `fs_search` invocation, keyboard result navigation,
 * and the per-hit context menu. Exposes an imperative focus handle to the
 * parent explorer.
 */
...
import { explorerRuntime, useExplorerPreferences } from "../../runtime";
import {
  cn,
  ContextMenu,
  ContextMenuContent,
  ...
  function ExplorerSearch(
    ...
    useEffect(() => {
      ...
      const handle = setTimeout(async () => {
        try {
          const res = (await explorerRuntime().files.search(
            {
              root: rootPath,
              query: q,
              limit: 200,
        ...
        {open ? (
          <div className="relative shrink-0 px-2 py-1.5 animate-in fade-in-0 slide-in-from-top-3 duration-200 ease-out">
            ...
            <Input
              ...
              className="h-7 rounded-lg border-border/60 bg-secondary pr-7 pl-6.5 text-xs"
            />
            {query ? (
              <button
                type="button"
