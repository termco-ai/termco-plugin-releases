...
// Decorative per-rig accent hues, distinct from the theme primary. Tuned to
// read on both light and dark surfaces. Indexed by RigMeta.color (opt-in).
...
export function accentFor(rig: Pick<RigMeta, "color">): string {
  const c = rig.color;
  if (c != null && c >= 0 && c < SPACE_COLORS.length) return SPACE_COLORS[c];
  return "var(--primary)";
}

export function rigInitial(name: string): string {
