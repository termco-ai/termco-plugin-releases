...
vi.mock("node:fs", async (original) => ({
  ...
  existsSync: (path: string) => path === "/virtual/gh",
  statSync: () => ({ isFile: () => true }), accessSync: () => {},
}));
...
it("handles a broken input pipe as a command failure without an unhandled stream error", async () => {
  ...
  const result = await forgeRun({ executable: "gh", args: ["api", "--hostname", "github.com", "--method", "POST", "repos/team/app/pulls/42/reviews", "--input", "-"], stdin: JSON.stringify({ commit_id: "a".repeat(40), event: "COMMENT", body: "feedback", comments: [] }) });
