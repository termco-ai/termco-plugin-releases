...
export function BlockWatermark({ leafId, subscribe }: Props) {
  ...
  return (
    ...
    >
      ...
      />
      <div className="grid grid-cols-[auto_auto] items-center gap-x-12 gap-y-3 text-sm">
...
function Key({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="inline-flex h-[22px] min-w-[22px] items-center justify-center rounded-md border border-border/45 bg-muted/30 px-1.5 font-sans text-xs font-medium text-muted-foreground/80">
      {children}
    </kbd>
  );
}
